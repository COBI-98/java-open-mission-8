...
- [ ] 도착 일시를 입력할 수 있다.
  - [ ] 방문 날짜와 시간을 문자열로 입력받는다.
  - [ ] `YYYY-MM-DD HH:mm` 형식이 아니면 예외를 발생시킨다.
  - [ ] 유효하지 않은 날짜/시간(2월 30일 등)이면 예외를 발생시킨다.
- [ ] 도착 터미널을 입력할 수 있다.
  - [ ] `T1`, `T2` 중 하나만 허용한다.
- [ ] 목적지 대략 위치를 입력할 수 있다.
  - [ ] 명동/시청, 홍대/마포, 강남/잠실, 인천/부천 등 **사전 정의된 구역 코드**로 입력받는다.
- [ ] 짐/동행자 상태를 입력할 수 있다.
  - [ ] 캐리어 개수는 0 이상 N 이하의 정수만 허용한다.
  - [ ] 동행자 유형(혼자, 가족, 아이 동반)을 선택할 수 있다.
- [ ] 이동 우선순위를 입력할 수 있다.
  - [ ] 시간/비용/편안함/경험 중 최소 1개, 최대 2개까지 선택할 수 있다.
- [ ] 언어 정보를 입력할 수 있다.
  - [ ] 선호 언어를 선택할 수 있다. (예: KOREAN, ENGLISH, JAPANESE, CHINESE)
